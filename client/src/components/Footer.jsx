...
const Footer = () => {
  ...
  const handleLinkClick = (item) => {
    switch (item) {
      ...
      case "Applied Jobs":
        ...
        break;
      default:
        // For other links, just scroll to top for now
        window.scrollTo(0, 0);
    }
  };
  ...
  return (
    <div className='mt-10 pb-5 flex gap-5 justify-around bg-mine-shaft-900 p-2 '>
        ...
        {
            footerLinks.map((foot,index)=><div key={index}>
            <div className='text-lg font-semibold text-cyan-/-aqua-500'>{foot.title}</div>
            {
                foot.link.map((item,ind)=>
                <div 
                  key={ind} 
                  onClick={() => handleLinkClick(item)}
                  className='text-mine-shaft-300 cursor-pointer hover:text-cyan-/-aqua-500 hover:translate-x-1 transition duration-300 ease-in-out'
                >
                  {item}
                </div>
                )
