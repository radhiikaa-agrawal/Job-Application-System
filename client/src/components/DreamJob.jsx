...
const DreamJob = () => {
  ...
  const handleSearch = () => {
    navigate('/find-jobs', { state: { title: jobTitle, jobType: jobType } });
  };
    ...
    <div className="bg-mine-shaft-950 min-h-screen flex flex-col lg:flex-row items-center justify-center p-6 ">
      ...
      <div className="flex flex-col w-full lg:w-[50%] gap-2 lg:mb-0">
        ...
        <div className="flex gap-3 mt-6 flex-wrap">
          <span className="text-mine-shaft-300 font-medium">Popular:</span>
          {['Data', 'Software', 'Java', 'Python', 'React'].map((tag) => (
            <button
              key={tag}
              onClick={() => {
                setJobTitle(tag);
                navigate('/find-jobs', { state: { title: tag } });
              }}
              className="px-3 py-1 bg-mine-shaft-800 text-cyan-500 rounded-full text-sm hover:bg-mine-shaft-700 transition-colors"
            >
              {tag}
            </button>
          ))}
